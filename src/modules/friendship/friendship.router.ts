...
export function makeFriendshipRouter(controller: FriendshipController): Router {
    ...
    router.get("/me", authenticate, authorize({ allow: [Roles.ADMIN, Roles.USER] }), asyncHandler(controller.getMine));
    ...
    router.delete("/requests", authenticate, authorize({ allow: [Roles.ADMIN, Roles.USER] }), asyncHandler(controller.removeRequest));
