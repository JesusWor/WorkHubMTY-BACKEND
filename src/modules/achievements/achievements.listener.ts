...
export type AchievementRule<K extends EventKey = EventKey> = {
    ...
    achievementId: number;
    description: string;
    handler: (ctx: AchievementRuleContext, ...args: GlobalEventMap[K]) => void | Promise<void>;
};

function defineRule<const K extends EventKey>(
    event: K,
    rule: Omit<AchievementRule<K>, "event">
): AchievementRule<K> {
    return {
        event,
        ...rule,
...
export const achievementRules = defineRules([
    ...
    defineRule("office.reservation.attendance_updated", {
        achievementId: 2,
        description: "¡Presente!",
        handler: async (ctx, payload) => {
            if (payload.reservation.attendance_status !== "CHECKED_IN") return;

            await Promise.all(
                payload.participants
                    .filter((p) => p.attendance_status === "CHECKED_IN")
                    .map((p) => ctx.addProgress(p.user_id, ctx.achievementId, 1))
            );
        },
    }),

    defineRule("office.reservation.created", {
        achievementId: 3,
        description: "CEO de reservaciones",
        handler: async (ctx, payload) => {
            const owner = payload.participants.find(
                (p) => p.ownership_priority === 0
            );
    ...
    defineRule("office.reservation.attendance_updated", {
        ...
        handler: async (ctx, payload) => {
            if (payload.reservation.attendance_status !== "NO_SHOW") return;

            await Promise.all(
                payload.participants
                    .filter((p) => p.attendance_status === "NO_SHOW")
                    .map((p) => ctx.addProgress(p.user_id, ctx.achievementId, 1))
            );
        },
    }),
    ...
    defineRule("parking.reservation.attendance_updated", {
        achievementId: 6,
        description: "Quemando llanta",
        handler: async (ctx, reservation) => {
            const startTime = reservation.start_time.getTime();
            const updatedAt = reservation.updated_at.getTime();

            const checkinWindowStart =
                startTime +
                CHECKIN_TOLERANCE_MINUTES * MINUTES_TO_MS -
                5 * MINUTES_TO_MS;

            const checkinWindowEnd =
                startTime + CHECKIN_TOLERANCE_MINUTES * MINUTES_TO_MS;

            if (
                reservation.attendance_status === "CHECKED_IN" &&
                checkinWindowStart < updatedAt &&
                updatedAt < checkinWindowEnd
            ) {
    ...
    defineRule("office.reservation.attendance_updated", {
        ...
        handler: async (ctx, payload) => {
            if (payload.reservation.lifecycle_status !== "FINALIZED") return;

            await Promise.all(
                payload.participants
                    .filter((p) => p.attendance_status === "REJECTED")
                    .map((p) => ctx.addProgress(p.user_id, ctx.achievementId, 1))
            );
        },
    }),
                ...
                payload.participants
                    .filter(
                        (p) =>
                            ...
                            payload.reservation.start_time.getHours() <= 9
                    )
                    .map((p) => ctx.addProgress(p.user_id, ctx.achievementId, 1))
            );
        },
    ...
    defineRule("office.reservation.attendance_updated", {
        ...
        handler: async (ctx, payload) => {
            ...
            await Promise.all(
                payload.participants
                    .filter((p) => p.attendance_status === "CANCELED")
                    .map((p) => ctx.addProgress(p.user_id, ctx.achievementId, 1))
            );
        },
    }),
    ...
    defineRule("office.reservation.attendance_updated", {
        ...
        handler: async (ctx, payload) => {
            if (payload.reservation.attendance_status !== "CHECKED_OUT") return;
...
export async function validateAchievementRules(
    ...
    if (missingIds.length > 0) {
        throw new Error(`Missing achievements required by ${errorLabel}: ${missingIds.join(', ')}`);
    }
}
