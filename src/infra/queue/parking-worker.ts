...
import {
    ...
    CheckoutJobData,
    bullmqConnection,
} from "../../infra/queue/parking-queue.js";
...
type JobData = NoShowJobData | CheckoutJobData;
...
async function handleNoShow(
    ...
    if (result.marked) {
        ...
        parkingEvents.emit("parking.reservation.noshow", result.reservation);
    } else {
...
async function handleAutoCheckout(
    ...
    switch (result.action) {
        case "checked_out":
            ...
            parkingEvents.emit("parking.reservation.attendance_updated", result.reservation);
            break;
        case "no_show_fallback":
            console.warn(
                `[parking-worker] Reservación ${reservationId} marcada NO_SHOW por fallback defensivo en auto-checkout. ` +
